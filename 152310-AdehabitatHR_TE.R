...
rm(list = ls())   #TE: It's always good to start from a clean slate
library(adehabitatHR)

library(tidyverse)  #TE

# TE: define what kind of variables are coming into the work space.
# this is helpful for checking your data also. Making sure only 
# expected inputs are in the file. Hadny for defining date and time.
# see strptime help for the definitions of the format (e.g., %d-%b-%Y
# and %H:%M:%S)
col.defs <- cols(Name = col_factor(),
...
#Reading csv
#turtles<-read.csv("data/152310-Filtered.csv",
#                  stringsAsFactors = FALSE)

# TE
turtles <- read_csv("data/152310-Filtered.csv",
                    col_types = col.defs)

#Creating a dataframe "xy" that contains name,x and y 
xy <- data.frame(Name = turtles$Name, 
                 X=turtles$Longitude, 
                 Y=turtles$Latitude)
